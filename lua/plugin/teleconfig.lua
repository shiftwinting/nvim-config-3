...
local cd = function(prompt_bufnr)
    ...
    vim.api.nvim_command('call CocAction("runCommand", "explorer.doAction", "3", {"name": "cd", "args": [getcwd()]})')
...
-- local inputdir = vim.fn.getcwd()
-- print('inputdir is ' . inputdir1)
...
local default_file_mappings = function(prompt_bufnr, map)
  map('i', '<C-1>', openInWindow1)
  map('i', '<C-2>', openInWindow2)
  map('i', '<C-3>', openInWindow3)
  map('i', '<C-e>', cd)
  map('i', '<C-r>', cdRoot)
  map('i', '<Esc>', actions.close)
  map('i', '<C-s>', actions.move_selection_next)
...
require('telescope.builtin').fd({ 
    ...
    find_command = { 'fd', '--type', 'd', '--type', 'l', '-L', '-H' },
    cwd = inputdir,
    previewer = false,
    width = 0.5,   
})    
...
elseif action == "find_dirs" then
    ...
    require('telescope.builtin').fd({ 
    attach_mappings = default_dir_mappings,   
    find_command = { 'fd', '--type', 'd', '--type', 'l', '--max-depth', '2', '-H', '-L', '--exclude', 'node_modules' },
    cwd = inputdir,
    previewer = false,
    width = 0.5,   
})                      
...
require('telescope.builtin').find_files {
    ...
    -- find_command = { 'fd', '--type', 'f', '-e', 'cpp', '-e', 'hpp', '-e', 'c', '-e', 'h', '-L' },
    shorten_path = false,
    initial_mode = "insert",
    cwd = inputdir,
    height = 10,
